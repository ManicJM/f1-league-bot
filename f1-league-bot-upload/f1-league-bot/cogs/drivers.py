...
class Drivers(commands.Cog):
    ...
    @driver.command(name="register", description="Register a driver in the league directory.")
    @app_commands.describe(
        name="Driver's display name",
        number="Car number",
        division="Which division/tier they race in",
        team="Team/constructor (optional)",
        gamertag="Their gamertag/PSN/Steam name (optional)",
        platform="Platform (optional)",
        member="Link this profile to a Discord member (optional; defaults to you)",
    )
    ...
    async def register(self, interaction: discord.Interaction, name: str,
                       number: app_commands.Range[int, 0, 999] = None,
        ...
        driver_id = database.add_driver(
            gid, name, number, team, gamertag,
            platform.value if platform else None,
            division_id=division_id, user_id=user.id,
        )
        div = database.get_division(gid, division_id) if division_id else None
        embed = discord.Embed(title="✅ Driver registered", color=BRAND)
        embed.add_field(name="Name", value=name, inline=True)
        embed.add_field(name="Number", value=str(number) if number is not None else "—", inline=True)
        embed.add_field(name="Division", value=div["name"] if div else "—", inline=True)
        embed.add_field(name="Team", value=team or "—", inline=True)
        embed.add_field(name="Gamertag", value=gamertag or "—", inline=True)
        embed.add_field(name="Platform", value=platform.value if platform else "—", inline=True)
        embed.add_field(name="Linked to", value=user.mention, inline=False)
        embed.set_footer(text=f"Driver id {driver_id}")
